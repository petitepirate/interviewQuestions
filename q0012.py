...
# you could climb any number from a set of positive integers X? For 
...
# The only ways to get to N = 3, is to first get to N = 1, and then 
# go up by 2 steps, or get to N = 2 and go up by 1 step. So f(3) = f(2) + f(1).

# Does this hold for N = 4? Yes, it does. Since we can only get to 
# the 4th step by getting to the 3rd step and going up by one, or by 
# getting to the 2nd step and going up by two. So f(4) = f(3) + f(2).
...
# Now, let's try to generalize what we've learned so that it works 
# if you can take a number of steps from the set X. Similar reasoning tells us that if X = {1, 3, 5}, then our algorithm should be f(n) = f(n - 1) + f(n - 3) + f(n - 5). If n < 0, then we should return 0 since we can't start from a negative number of steps.
