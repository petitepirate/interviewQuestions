...
# 'A', you should return the list ['A', 'B', 'C', 'A', 'C'] even though ['A', 'C', 'A', 'B', 'C']
# is also a valid itinerary. However, the first one is lexicographically smaller.

# __________________________________________________________________________________________________
# Solution
# This problem is similar to the N queens problem a few days ago: we have a desired final state
# (all the flights are used up), we can construct partial itineraries and reject them, and at
# each step we have potentially multiple avenues to explore. That suggests that backtracking
# is again a very likely candidate for solving our problem.
...
# For each flight, temporarily add the destination to our itinerary and remove it from the flight
# list. Then call ourselves recursively with the new itinerary and flight list.
...
# To speed this up, we'll store all the flights into a dictionary with the origin as a key and a
# list of flight destinations from that origin as the value. Then we can look up our options in
...
def get_itinerary(flights, start):
    ...
    def visit(flight_map, total_flights, current_itinerary):
        ...
        for i, flight in enumerate(flight_map[last_stop]):
            ...
            potential_itineraries.extend(
                visit(flight_map, total_flights, current_itinerary))
